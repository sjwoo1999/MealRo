...
MealRo는 기존의 서구권 중심 공식(Mifflin-St Jeor)의 한계를 극복하기 위해, **2025 한국인 영양소 섭취기준(KDRI)**을 기반으로 한 독자적인 **"Precision EER Algorithm"**을 탑재했습니다.

#### 1) Science Basis (학술적 배경)
*   **Ministry of Health and Welfare (2025)**: 한국인의 체격을 반영한 최신 에너지 필요량 추정식(EER)을 적용했습니다.
*   **Ndahimana et al. (2018) & Porter et al. (2025)**: 체성분(InBody) 데이터를 활용한 고정밀 대사량 산출 로직을 구현했습니다.

#### 2) Algorithm Logic (알고리즘 상세)
단순한 칼로리 계산을 넘어, 사용자의 **생애 주기(Life Cycle)**와 **신체 구성(Body Composition)**을 입체적으로 분석합니다.

*   **A. 이원화된 에너지 산출 (Dual-Track EER)**
    *   **Track 1: General (표준)**: 나이, 성별, 신장, 체중을 기반으로 한 KDRI 표준 공식을 따릅니다.
    *   **Track 2: Precision (정밀)**: **제지방량(FFM)** 데이터가 입력되면, 근육량에 따른 대사 차이를 반영하여 오차 범위를 획기적으로 줄입니다.
    *   *Formula Example (Adult, FFM)*: `EER = (19.324 × FFM + 4.588 × Weight – 2.479 × Age) × PA`

*   **B. 적응형 단백질 타겟팅 (Adaptive Protein Targeting)**
    단백질 목표량은 "단일 권장량"이 아닌, 사용자의 상태에 따라 3단계로 동적 조정됩니다.
    1.  **Baseline (성인 표준)**: `0.91g/kg` (질소 평형 유지 최소량)
    2.  **Anti-Sarcopenia (노년층)**: `1.2g/kg` (근감소증 예방을 위한 상향 조정, Porter et al.)
    3.  **Anabolic/Maternal (특수 목적)**:
        *   임신/수유부: 태아 발달 및 모유 생성을 위한 안전 부가량(+10~25g) 자동 합산
        *   고강도 운동/증량: 근합성을 위한 `1.6~2.0g/kg` 적용 (Morton et al.)
